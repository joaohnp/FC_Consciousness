...
def test_all_distributions(AW, NREM, REM):
    kw_stat, kw_pvalue = stats.kruskal(AW, NREM, REM)
    return {
        "kruskal_stat": kw_stat,
        "kruskal_pvalue": kw_pvalue,
    }
...
def get_distribution_fig(AW, NREM, REM, labels=["AW", "NREM", "REM"], fontsize=16):
    ...
    ax2.hist(AW, bins=20, alpha=0.2, label=labels[0], weights=weights1)
    ax2.hist(NREM, bins=20, alpha=0.2, label=labels[1], weights=weights2)
    ax2.hist(REM, bins=20, alpha=0.2, label=labels[2], weights=weights3)
...
for chosen_results in results_sessions:
    ...
    with open(sleepScores_path / f"{chosen_results[0:-4]}.pickle", "rb") as fp:
    ...
    avg_NREM = NREM.mean()
    avg_REM = REM.mean()
    output = test_all_distributions(AW=AW, NREM=NREM, REM=REM)
    df_results = pd.DataFrame(output, index=[0])
    df_results["Session"] = chosen_results
    df_results["lower than 0.05"] = df_results["kruskal_pvalue"] < alpha
    df_results["avg_aw"] = avg_AW
    df_results["avg_nrem"] = avg_NREM
    df_results["avg_rem"] = avg_REM
    all_results_df = pd.concat([all_results_df, df_results], ignore_index=True)
    dump_path = projectPath / "results_interareal.pkl"
    all_results_df.to_pickle(dump_path)
    print(f"Added results for session {chosen_results}")

# Plotting exemplar session
chosen_results = results_sessions[-1]
with open(magnitudesPath / chosen_results, "rb") as fp:
    df_curbd = pickle.load(fp)
...
save_path = figures_path / "fig7_panel_a.svg"
AW = df_curbd[(df_curbd["scores"] == 1) | (df_curbd["scores"] == 2)]["no_eye"].dropna()
NREM = df_curbd[df_curbd["scores"] == 3]["no_eye"].dropna()
REM = df_curbd[df_curbd["scores"] == 4]["no_eye"].dropna()
...
output = test_all_distributions(AW=AW, NREM=NREM, REM=REM)
fig = get_distribution_fig(AW, NREM, REM, labels=["AW", "NREM", "REM"])
...
for chosen_results in results_sessions:
    ...
    NREM = df_curbd[df_curbd["scores"] == 3]["no_eye"].dropna()
    REM = df_curbd[df_curbd["scores"] == 4]["no_eye"].dropna()
    toplot = [AW.median(), NREM.median(), REM.median()]
...
fig.suptitle("Interareal connections - all epochs", fontsize=fontsize + 3)
save_path_all = figures_path / "fig_7_panel_b.svg"
plt.savefig(
    save_path_all,
    transparent=True,  # Removes white background
    format="svg",  # Ensures SVG format
