...
conditions = ["NREMvsREM", "NREMvsAWAKE"]
# conditions = ["NREMvsAWAKE"]

all_results_df = pd.DataFrame()
for condition in conditions:
    resultsPath = os.path.join(projectPath, "results", condition)
    prcnt = os.listdir(resultsPath)
    for chosen_prcnt in prcnt:
        p_values = []
        results_percent = os.listdir(os.path.join(resultsPath, chosen_prcnt))
        for chosen_pkl in results_percent:
            file = open(os.path.join(resultsPath, chosen_prcnt, chosen_pkl), "rb")
        ...
        dump_path = os.path.join(projectPath, "combined_p.pkl")
...
## Printing results for a single session
for condition in conditions:
    resultsPath = os.path.join(projectPath, "results", condition)
    prcnt = os.listdir(resultsPath)
    for chosen_prcnt in prcnt:
        ...
        save_path = os.path.join(
            projectPath, "Figures", f"exemplar_session{chosen_prcnt}_{condition}.svg"
        )
        plt.savefig(
...
def get_population_figure(df_population, fontsize=16):
    ...
    fig, axes = plt.subplots(
        len(conditions), len(cutoffs), figsize=(18, 12), constrained_layout=True
    )
    for i, condition in enumerate(conditions):
        for j, cutoff in enumerate(cutoffs):
            # Filter data for this condition and cutoff
            subset = df_population[
                (df_population["Condition"] == condition)
                & (df_population["Cutoff"] == cutoff)
            ]
            # Create scatter plot
            ax = axes[i, j]
...
for condition in conditions:
    ...
    for chosen_prcnt in prcnt:
        p_values = []
        for results in os.listdir(os.path.join(resultsPath, chosen_prcnt)):
            chosen_pkl = results
            file = open(os.path.join(resultsPath, chosen_prcnt, chosen_pkl), "rb")
            df = pd.DataFrame.from_dict(pickle.load(file))
            print(df)
