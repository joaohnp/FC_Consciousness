...
def trainMultiRegionRNN(
    ...
    for nRun in range(0, nRunTot):
        ...
        for tt in range(1, len(tRNN)):
            ...
            JR = (
                J.dot(RNN[:, tt]).reshape((number_units, 1))
        ...
        pVar = (
            1
            - (distance / (math.sqrt(len(iTarget) * len(tData)) * stdData))
            ** 2
        )
        pVars.append(pVar)
        chi2s.append(chi2)
        ...
        if fig:
            ...
            ax.plot(tRNN, RNN[iTarget[idx], :], "grey")
            ax.plot(tData, Adata[iTarget[idx], :], "green", alpha=0.7)
            ax.set_title(f"Run {nRun} pvar {pVars[-1]}")


            # Your existing logic
            # if len(pVars) > 100 and pVars[-1] > 0.2:
            #     # Check if the file already exists
            #     if not os.path.exists("my_plot.pkl"):
            #         print("Found a good example and saving the figure!")
            #         with open("my_plot.pkl", "wb") as file:
            #             pickle.dump(fig, file)
            #     else:
            #         print("Figure file already exists, not saving again.")

            fig.show()
            plt.pause(0.0001)
            if nRun == nRunTot-1:
                currentDateAndTime = datetime.now()
                currentTime = currentDateAndTime.strftime("%d%m%Y_%H%M")
                projectPath = '/home/joaohnp/github/curbd_jp/'
                path_tosave = os.path.join(projectPath, "Figures", 
                                           "training",
                                           f"{currentTime}.svg")
                dpi = 300
                fig.savefig(
...
def threeRegionSim(
    ...
    newmat = np.tile(
        hBump[:, 1, np.newaxis], (1, math.ceil(leadTime / dtData))
    )
